...
import com.example.ecommercestore.databinding.ProductRvItemBinding
import com.example.ecommercestore.models.Product

class BestProductsAdapter:Adapter<BestProductsAdapter.BestProductsHolder>(){

    inner class BestProductsHolder(private val binding:ProductRvItemBinding) : ViewHolder(binding.root){
        fun bind(product:Product){
            binding.apply {
                Glide.with(itemView).load(product.images[0]).into(imgBestDealsProduct)
                tvBestDealsProductName.text = product.name
                ...
                tvBestDealsPrice.text = product.price.toString()
            }
        }
    }
